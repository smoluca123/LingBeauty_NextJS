...
import { generateSlug, generateSku } from '../product-form/product-form-helpers';

// ============ Helpers: map BE data → form data ============

function mapProductToFormData(product: IAdminProductDataType): IProductFormData {
  return {
    name: product.name,
    slug: product.slug,
    sku: product.sku,
...
export function useEditProductForm(product: IAdminProductDataType) {
  const initialFormData = useMemo(() => mapProductToFormData(product), [product]);
  ...
  const updateField = useCallback(
    <K extends keyof IProductFormData>(field: K, value: IProductFormData[K]) => {
  ...
  const addVariant = useCallback(() => {
    setFormData((prev) => ({
      ...
      variants: [
        ...prev.variants,
        { sku: '', name: '', color: '', size: '', type: '', displayType: 'COLOR', price: 0, quantity: 0, lowStockThreshold: 10 },
      ],
    }));
  }, []);
  ...
  const buildUpdatePayload = useCallback((): IUpdateProductPayload => {
    ...
    if (formData.shortDesc !== initialFormData.shortDesc) payload.shortDesc = formData.shortDesc;
    if (formData.description !== initialFormData.description) payload.description = formData.description;
    if (formData.basePrice !== initialFormData.basePrice) payload.basePrice = formData.basePrice;
    if (formData.comparePrice !== initialFormData.comparePrice) payload.comparePrice = formData.comparePrice;
    if (formData.weight !== initialFormData.weight) payload.weight = formData.weight;
    if (formData.isActive !== initialFormData.isActive) payload.isActive = formData.isActive;
    if (formData.isFeatured !== initialFormData.isFeatured) payload.isFeatured = formData.isFeatured;
    if (formData.brandId !== initialFormData.brandId) payload.brandId = formData.brandId || undefined;
    ...
    const variantsChanged =
      JSON.stringify(formData.variants) !== JSON.stringify(initialFormData.variants);
    if (variantsChanged) {
