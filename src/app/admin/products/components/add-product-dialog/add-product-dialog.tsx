...
export function AddProductDialog({
    ...
    .flatMap((brandRoot) =>
      (brandRoot.children ?? [])
        ...
        .map((child) => ({
          ...
          name:
            (child as { brand?: { name?: string } }).brand?.name ?? child.name,
          slug:
            (child as { brand?: { slug?: string } }).brand?.slug ?? child.slug,
          description:
            (child as { brand?: { description?: string } }).brand
              ?.description ?? '',
          isActive: child.isActive,
          logoMediaId:
            (child as { brand?: { logoMedia?: { id?: string } } }).brand
              ?.logoMedia?.id ?? '',
          website: null as null,
          logoMedia:
            (
              child as {
                brand?: { logoMedia?: (typeof brandsFromAPI)[0]['logoMedia'] }
              }
  ...
  const brandNamesFromAPI = new Set(
    brandsFromAPI.map((b) => b.name.toLowerCase()),
  )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-7xl max-w-[90vw] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          <ProductTypeSection formData={formData} onFieldChange={updateField} />
          ...
          <PricingSection formData={formData} onFieldChange={updateField} />

          {/* Only show variants for inventory products */}
          ...
          <OptionsSection formData={formData} onFieldChange={updateField} />
        </div>
