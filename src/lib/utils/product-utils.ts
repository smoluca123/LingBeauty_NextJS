/**
 * Product utilities for checking stock status and inventory
 */
 ...
 * @param inventory - The inventory record to check
 * @returns True if inventory is out of stock
 */
 ...
 * @param inventory - The inventory record to check
 * @returns True if inventory is low stock
 */
 ...
 * @example
 * getIsOutOfStock(productWithNoStock) // true
 * getIsOutOfStock(productWithStock) // false
 */
 ...
 * @example
 * getIsLowStock(productWithLowStock) // true
 * getIsLowStock(productOutOfStock) // false
 * getIsLowStock(productWithGoodStock) // false
 */
export const getIsLowStock = (product: IProductDataType): boolean => {
  ...
  if (variants.length > 0) {
    return variants.some(
      (v) =>
        !isInventoryOutOfStock(v.inventory) && isInventoryLowStock(v.inventory),
    )
  }
