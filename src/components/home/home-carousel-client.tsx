...
export function HomeCarouselClient({ banners }: HomeCarouselClientProps) {
  ...
  const { carouselSlides, sideBanners } = useMemo(() => {
    ...
    return {
      ...
      sideBanners: sideBannerList.length > 0 ? sideBannerList : FALLBACK_SIDE,
    };
  }, [banners]);
  ...
  return (
    <div className="flex w-full flex-row gap-3 md:gap-4 lg:w-md lg:flex-col">
      {banners.map((banner) => (
        <div
          key={banner.id}
          className="flex-1 rounded-xl border bg-linear-to-br p-4 sm:p-5 shadow-sm"
          style={{
